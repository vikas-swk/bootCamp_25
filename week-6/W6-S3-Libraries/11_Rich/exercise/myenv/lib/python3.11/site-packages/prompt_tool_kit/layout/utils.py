...
def find_window_for_buffer_name(cli, buffer_name):
    """
    Look for a :class:`~prompt_tool_kit.layout.containers.Window` in the Layout
    that contains the :class:`~prompt_tool_kit.layout.controls.BufferControl`
    for the given buffer and return it. If no such Window is found, return None.
    """
    from prompt_tool_kit.interface import CommandLineInterface
